...
test_qExportWig_qQCReport <- function()
{
    ...
    checkTrue(is.list(resqc))
    checkTrue(length(resqc) == 8L)
    checkTrue(is.list(resqc$nuclByCycle) &&
                  is.matrix(resqc$nuclByCycle[[1]]) &&
                  is.matrix(resqc$nuclByCycle[[2]]))
    checkTrue(all(dim(resqc$nuclByCycle) == c(5L, 36L)))
    checkTrue(all(dim(resqc$nuclByCycle[[1]]) == c(5L, 36L)))
    checkTrue(all(dim(resqc$nuclByCycle[[2]]) == c(5L, 36L)))
    unlink(tmppdf)
    
    # check qExportWig
    # ... arguments
    checkException(qExportWig("error"))
    checkException(qExportWig(project, collapseBySample = FALSE, strand = c("+", "-")))
    checkException(qExportWig(project, file = "test.wig"))
    checkException(qExportWig(project, createBigWig = TRUE, file = c("w1.wig","w2.wig")))
    checkException(qExportWig(project, file = c("w1.wig.bz2","w2.wig.bz2")))
    checkException(qExportWig(project, binsize = c(100, 200)))
    checkException(qExportWig(project, binsize = -100))
    checkException(qExportWig(project, shift = NA))
    checkException(qExportWig(project, shift = c(10, 20, 30)))
    checkException(qExportWig(project, scaling = c(1,2,3)))
    checkException(qExportWig(project, scaling = TRUE, log2p1 = "yes"))
    checkException(qExportWig(project, scaling = FALSE, colors = "gray", includeSecondary = "yes"))
    checkException(qExportWig(project, absIsizeMin = 100))
