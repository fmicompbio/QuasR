...
The sample file is a tab delimited text file, with the two columes 'SampleName' and 'FileName' for single-end and three columes 'SampleName', 'FileName' 'FileNameMate' for a paired-end experiment.

\begin{tabular}{|ll|}
\hline
SampleName&FileName\\
wt&filenameWt.fastq\\
ko&filenameKo1.fastq\\
ko&filenameKo2.fastq\\
\hline
\end{tabular}
