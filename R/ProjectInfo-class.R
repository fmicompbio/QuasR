...
ProjectInfo <- function(sampleFile="Sample.txt", genome=".", annotationFile="Annotation.txt", aligner="Rbowtie", projectName="projectinfo", path=".", ...){
  project <- new("ProjectInfo", projectName, path, ...)
  #if(missing(path))
  #  project@path <- dirname(sampleFile)
  # set sample
  project@samples <- readSamples(sampleFile)
  # set annotation
  project@annotations <- readAnnotations(annotationFile)
  # set genome
  project@genome <- loadGenome(genome)
  # set aligner
  project@aligner <- Aligner(aligner)
  # set index
  project@index <- loadIndex(project@genome, project@aligner)
  return(project)
}
