...
%\newcommand{\QuasR}{\Rpackage{QuasR}}  % adjusted to BiocStyle
%\newcommand{\QuasR}{\Biocpkg{QuasR}}
...
% BiocStyle captures \title{} for use in headers since we put an image
% into \title{}, we have to overwrite header with plain text
\bioctitle[An Introduction to \Rpackage{QuasR}]%
    {An Introduction to \QuasRlogo{}}
\author{Anita Lerch, Dimos Gaidatzis and Michael Stadler}
\date{Modified: January 17 2014. Compiled: \today}
...
The \Biocpkg{QuasR} package (short for \underline{Qu}antify and \underline{a}nnotate \underline{s}hort reads in \underline{R}) integrates the functionality of several \R{} packages (such as \Biocpkg{IRanges} \cite{IRanges} and \Biocpkg{Rsamtools}) and external software (e.g. \software{bowtie}, through the \Biocpkg{Rbowtie} package). The package aims to cover the whole analysis workflow of typical ultra-high throughput sequencing experiments, starting from the raw sequence reads, over pre-processing and alignment, up to quantification. A single \R{} script can contain all steps of a complete analysis, making it simple to document, reproduce or share the workflow containing all relevant details.

The current \Biocpkg{QuasR} release supports the analysis of single read and paired-end ChIP-seq (chromatin immuno-precipitation combined with sequencing), RNA-seq (gene expression profiling by sequencing of RNA) and Bis-seq (measurement of DNA methylation by sequencing of bisulfite-converted genomic DNA) experiments. It has been successfully used with data from Illumina GA/GA II/MiSeq/HiSeq, 454 Life Technologies and SOLiD sequencers, the latter by using bam files created externally of \Biocpkg{QuasR}.

\section{Preliminaries}

\subsection{Citing \Biocpkg{QuasR}}
If you use \Biocpkg{QuasR} \cite{QuasR} in your work, you can cite it as follows:
<<cite, eval=TRUE>>=
citation("QuasR")
@
...
\Biocpkg{QuasR} is a package for the \R{} computing environment and it is assumed that you have already installed \R{}. See the \R{} project at \url{http://www.r-project.org}. To install the latest version of \Biocpkg{QuasR}, you will need to be using the latest version of \R{}. \Biocpkg{QuasR} is part of the Bioconductor project at \url{http://www.bioconductor.org}. To get \Biocpkg{QuasR} together with its dependencies you can use
<<install, eval=FALSE>>=
source("http://www.bioconductor.org/biocLite.R")
biocLite("QuasR")
@

Bioconductor works on a 6-monthly official release cycle. As with other Bioconductor packages, there are always two versions of \Biocpkg{QuasR}. Most users will use the current official release version, which will be installed by \Rcode{biocLite} if you are using the current version of \R{}. There is also a developmental version of \Biocpkg{QuasR} that includes new features due for the next official release. The developmental version will be installed if you are using the developmental version of \R{}. The official release version always has an even second number (for example 0.2.1), whereas the developmental version has an odd second number (for example 0.3.6).

\subsection{Loading of \Biocpkg{QuasR} and other required libraries}
In order to run the code examples in this vignette, the \Biocpkg{QuasR} library and a few additional libraries need to be loaded:
...
Most questions about \Biocpkg{QuasR} will hopefully be answered by the documentation or references. If you've run into a question which isn't addressed by the documentation, or you've found a conflict between the documentation and software itself, then there is an active support community which can offer help.
The authors of the package (maintainer: \Sexpr{maintainer("QuasR")}) always appreciate receiving reports of bugs in the package functions or in the documentation. The same goes for well-considered suggestions for improvements. 
Any other questions or problems concerning \Biocpkg{QuasR} should be sent to the Bioconductor mailing list \url{bioconductor@stat.math.ethz.ch}. To subscribe to the mailing list, see \url{https://stat.ethz.ch/mailman/listinfo/bioconductor}. Please send requests for general assistance and advice to the mailing list rather than to the individual authors. Users posting to the mailing list for the first time should read the helpful posting guide at \url{http://www.bioconductor.org/doc/postingGuide.html}. Note that each function in \Biocpkg{QuasR} has it's own help page, as described in the section \ref{sec:Rintro}. Mailing list etiquette requires that you read the relevant help page carefully before posting a problem to the list.
...
The structure of this vignette and in particular this section is based on the excellent user guide of the \Biocpkg{limma} package, which we would like to hereby acknowledge. \R{} is a program for statistical computing. It is a command-driven language meaning that you have to type commands into it rather than pointing and clicking using a mouse. In this guide it will be assumed that you have successfully downloaded and installed \R{} from \url{http://www.r-project.org} as well as \Biocpkg{QuasR} (see section \ref{sec:Installation}). A good way to get started is to type
<<help1, eval=FALSE>>=
help.start()
@ 
at the \R{} prompt or, if you're using \R{} for Windows, to follow the drop-down menu items \textit{Help} $\succ$ \textit{Html help}. Following the links \textit{Packages} $\succ$ \Biocpkg{QuasR} from the html help page will lead you to the contents page of help topics for functions in \Biocpkg{QuasR}.
Before you can use any \Biocpkg{QuasR} commands you have to load the package by typing
...
x
@ 
We hope that you can use \Biocpkg{QuasR} without having to spend a lot of time learning about the \R{} language itself but a little knowledge in this direction will be very helpful, especially when you want to do something not explicitly provided for in \Biocpkg{QuasR} or in the other Bioconductor packages. For more details about the \R{} language see \textit{An Introduction to R} which is available from the online help. For more background on using \R{} for statistical analysis see \cite{Dalgaard}.


\subsection{Sample \Biocpkg{QuasR} session}
\label{sec:SampleSession}
This is a quick overview of what an analysis could look like for users preferring to jump right into an analysis. The example uses data that is provided with the \Biocpkg{QuasR} package, which is first copied to the current working directory, into a subfolder called \Rcode{"extdata"}:
...
\newpage
\section{\Biocpkg{QuasR} Overview}
The following scheme shows the major components of \Biocpkg{QuasR} and their relationships:
\begin{figure}[!h]
\begin{center}
...
\Biocpkg{QuasR} works with data (sequences and alignments, reference genome, etc.) that are stored as files on your storage (the gray cylinder on the lower left of Figure \ref{fig:QuasR-scheme}, see section \ref{sec:FileStorage} for details on storage locations). \Biocpkg{QuasR} does not need a database management system, or these files to be named and organized according to a specific scheme.

In order to keep track of directory paths during an analysis, \Biocpkg{QuasR} makes use of a \Robject{qProject} object that is returned by the \Rfunction{qAlign} function, which at the minimum requires two inputs: the name of a samples text file (see section \ref{sec:SampleFile} for details), and the reference genome for the alignments (see section \ref{sec:refGenome}).
...
Apart from \Rfunction{qExportWig} and \Rfunction{qQCReport}, which generate wig files and pdf reports, \Rfunction{qAlign} is the only function in \Biocpkg{QuasR} that stores files on the disk (see section \ref{sec:qAlign} for details). All files generated by \Rfunction{qAlign} are listed here by type, together with their default location and how locations can be changed.
\begin{itemize}
  \item \textit{Temporary files} (default: \Rcode{tempdir()}): Temporary files include reference genomes in \fasta{} format, decompressed input sequence files, and temporary alignments in text format, and can require a large amount of disk space. By default, these files will be written to the temporary directory of the \R{} process (as reported by \Rcode{tempdir()}). If using \Rfunarg{clObj} for parallel processing, this may be the \Rcode{tempdir()} from the cluster node(s). An alternative location can be set using the \Rcode{TMPDIR} environment variable (see \Rcode{?tempdir}).
  \item \textit{Alignment files (\bam{} format)} (default: same directory as the input sequence files): Alignments against reference genome and auxiliary targets are stored in \bam{} format in the same directory that also contains the input sequence file (listed in \Rfunarg{sampleFile}). Please note that if the input sequence file correspond to a symbolic link, \Biocpkg{QuasR} will follow the link and use the directory of the original file instead. An alternative directory can be specified with the \Rfunarg{alignmentsDir} argument from \Rfunction{qAlign}, which will store all \bam{} in that directory even if the input sequence files are located in different directories.
  \item \textit{Alignment index files} (default: depends on \Rfunarg{genome} and \Rfunarg{snpFile} arguments): Many alignment tools including \software{bowtie} require an index of the reference sequence to perform alignments. If necessary, \Rfunction{qAlign} will build this index automatically and store it in a default location that depends on the \Rfunarg{genome} argument:
    \begin{itemize}
      \item \Rcode{BSgenome}: If \Rfunarg{genome} is the name of a \Biocpkg{BSgenome} package (such as \nolinkurl{"BSgenome.Hsapiens.UCSC.hg19"}), the index will be stored as a new \R{} package in the default library path (as reported by \Rcode{.libPaths()[1]}, see \Rcode{?install.packages} for details). The name of this index package will be the name of the original \Biocpkg{BSgenome} package with a suffix for the index type, for example \nolinkurl{"BSgenome.Hsapiens.UCSC.hg19.Rbowtie"}.
      \item \fasta{}: If \Rfunarg{genome} refers to a reference genome file in \fasta{} format, the index will be stored in a subdirectory at the same location. Similarly, the indices for files listed in \Rfunarg{auxiliaryFile} are store at the location of these files. For example, the \Rcode{Rbowtie} index for the genome at \nolinkurl{"./genome/mm9.fa"} is stored in \nolinkurl{"./genome/mm9.fa.Rbowtie"}.
      \item \textit{Allele-specific analysis}: A special case is the allele-specific analysis, where reference and alternative alleles listed in \Rfunarg{snpFile} (e.g. \nolinkurl{"./mySNPs.tab"}) are injected into the \Rfunarg{genome} (e.g. \nolinkurl{"BSgenome.Mmusculus.UCSC.mm9"}) to create two variant genomes to be indexed. These indices are saved at the location of the \Rfunarg{snpFile} in a directory named after \Rfunarg{snpFile}, \Rfunarg{genome} and the index type (e.g. \nolinkurl{"./mySNPs.tab.BSgenome.Mmusculus.UCSC.mm9.A.fa.Rbowtie"}).
    \end{itemize}
\end{itemize}
...
These example files are also contained in the \Biocpkg{QuasR} package and may be used as templates. The path of the files can be determined using:
...
\item \textbf{\fastq{}} files have names that end with '.fq' or '.fastq'. They contain sequences and corresponding base qualities and will be aligned by default using these qualities. The encoding scheme of base qualities is automatically detected for each individual fastq file.
\item \textbf{\bam{}} files have names that end with '.bam'. They can be used if the sequence reads have already been aligned outside of \Biocpkg{QuasR}, and \Biocpkg{QuasR} will only be used for downstream analysis based on the alignments contained in the \bam{} files. This makes it possible to use alignment tools that are not available within \Biocpkg{QuasR}, but making use of this option comes with a risk and should only be used by experienced users. For example, it cannot be guaranteed any more that certain assumptions made by \Rfunction{qCount} are fulfilled by the external aligner (see below). In addition, since the data provided to \Biocpkg{QuasR} has already been processed, quality control plots that are based on unprocessed raw data will be missing from the output of \Rfunction{qQCReport}. When using external \bam{} files, we recommend to use files which contain only one alignment per read. This may also include multi-hit reads, for which one of the alignments is randomly selected. This allows \Biocpkg{QuasR} to count the total number of reads by counting the total number of alignments. Furthermore, if the \bam{} files also contain the unmapped reads, \Biocpkg{QuasR} will be able to calculate the fraction of mapped reads. For bisulfite samples we require ungapped alignments stored in unpaired or paired \emph{ff} orientation (even if the input reads are \emph{fr}). For allele-specific \bam{} files, \Biocpkg{QuasR} requires an additional tag for each alignment called \texttt{XV} of type \texttt{A} (printable character) with the possible values \texttt{R} (Reference), \texttt{U} (Unknown) and \texttt{A} (Alternative).
\end{itemize}
...
\subsubsection*{Working only with \bam{} files after performing alignments}
Once alignments have been created, most analyses will only require the \bam{} files and will not access the original raw sequence files anymore. However, re-creating a \Rcode{qProject} object by a later identical call to \Rfunction{qAlign} will still need access to the raw sequences to verify consistency between raw data and alignments. It may be desirable to remove this dependency, for example to archive or move away the raw sequence files and to reclaim used disk space.

This can be achieved using the following procedure involving two sequential calls to \Rfunction{qAlign}. First, \Rfunction{qAlign} is called with the orignial sample file (\Rcode{sampleFile1}) that lists the raw sequence files, and subsequently with a second sample file (\Rcode{sampleFile2}) that lists the \bam{} files generated in the first call. Such a second sample file can be easily generated given the \Rcode{qProject} object (\Rcode{proj1}) returned by the first call:
...
and is available from your \Biocpkg{QuasR} installation at
...
\label{sec:refGenome}
Sequence reads are primarily aligned against the reference genome. If necessary, \Biocpkg{QuasR} will create an aligner index for the genome. The reference genome can be provided in one of two different formats:
\begin{itemize}
\item{\textbf a string}, referring to the name of a \Biocpkg{BSgenome} package:
<<selectGenomeBSgenome, eval=TRUE>>=
available.genomes()
genomeName <- "BSgenome.Hsapiens.UCSC.hg19"
@
In this example, the BSgenome package \Rcode{"BSgenome.Hsapiens.UCSC.hg19"} refers to an unmasked genome; alignment index and alignments will be performed on the full unmasked genome sequence (recommended). If using a masked genome (e.g. \Rcode{"BSgenome.Hsapiens.UCSC.hg19.masked"}), masked regions will be replaced with \Rcode{"N"} bases, and this hard-masked version of the genome will be used for creating the alignment index and further alignments.
...
By default, newly generated \bam{} files will be stored at the location of the input sequence files, which should be writable and have sufficient capacity (an alternative location can be specified using the \Rfunarg{alignmentsDir} argument). Make also sure that you have sufficient temporary disk space for intermediate files in \Rcode{tempdir()} (see section \ref{sec:qAlign}). We start by aligning the reads using \Rfunction{qAlign}:
...
The counts returned by \Rfunction{qCount} are the raw number of alignments per sample and region, without any normalization for the query region length, or the total number of aligned reads in a sample. As expected, we can find H3K4me3 signal at promoters of a subset of the genes with CpG island promoters, which we can visualize with help of the \Biocpkg{Gviz} package:
...
<<ChIP_visualizeProfile, eval=TRUE, fig=TRUE, width=8, height=4.5>>=
...
\subsubsection{Using a \Biocpkg{BSgenome} package as reference genome}
\Biocpkg{QuasR} also allows using of \Biocpkg{BSgenome} packages instead of a \fasta{} file as reference genome (see section \ref{sec:refGenome}). To use a \Biocpkg{BSgenome}, the \Rfunarg{genome} argument of \Rfunction{qAlign} is set to a string matching the name of a \Biocpkg{BSgenome} package, for example \nolinkurl{"BSgenome.Hsapiens.UCSC.hg19"}. If that package is not already installed, \Rfunction{qAlign} will check if it is available from \url{bioconductor.org} and download it automatically. The corresponding alignment index will be saved as a new package, named after the original \Biocpkg{BSgenome} package and the aligner used to build the index, for example \nolinkurl{BSgenome.Hsapiens.UCSC.hg19.Rbowtie}.

The code example below illustrates the use of a \Biocpkg{BSgenome} reference genome for the same example data as above. Running it for the first time will take several hours in order to build the aligner index: \label{obj:proj1}
...
\subsubsection{Calculation of RPKM expression values}
...
\subsubsection{Analysis of alternative splicing: Quantification of exon-exon junctions}
Exon-exon junctions can be quantified by setting \Rcode{reportLevel="junction"}. In this case, \Rfunction{qCount} will ignore the \Rfunarg{query} argument and scan all alignments for any detected splices, which are returned as a \Robject{GRanges} object: The region start and end coordinates correspond to the first and last bases of the intron, and the counts are returned in the \Rcode{mcols()} of the \Robject{GRanges} object. Alignments that are identically spliced but reside on opposite strands will be quantified separately. In an unstranded RNA-seq experiment, this may give rise to two separate counts for the same intron, one each for the supporting alignments on plus and minus strands.
...
\subsection{smRNA-seq: small RNA and miRNA expression profiling}
\label{sec:miRNA}
...
\subsubsection{Preprocessing of small RNA (miRNA) reads}
Again, we start the example workflow by copying the example data files into the current working directly, into a subfolder called \Rcode{"extdata"}.
...
We can remove those adapter sequences using \Rfunction{preprocessReads} (see section~\ref{sec:preprocessReads} on page~\pageref{sec:preprocessReads} for more details), which for each input sequence file will generate an output sequence file containing appropriately truncated sequences. In the example below, we get the input sequence filenames from \Rcode{sampleFile}, and also prepare an updated \Rcode{sampleFile2} that refers to newly generated processed sequence files: 
...
The miRNA reads in \Rcode{mirna\_1.fa} are by the way synthetic sequences and do not correspond to any existing miRNAs. As you can see above from the return value of \Rfunction{preprocessReads}, all reads matched to the 3'-adapter and were therefore truncated, reducing their length to roughly the expected 22nt:
<<miRNA_lengthes, eval=TRUE, fig=TRUE, width=6, height=4.5>>=
# get read lengths
library(Biostrings)
oldwd <- setwd(dirname(sampleFile))
...
\subsubsection{Alignment of small RNA (miRNA) reads}
Next, we create alignments using \Rfunction{qAlign}. In contrast to the general RNA-seq workflow (section \ref{sec:RNA}), alignment time can be reduced by using the default unspliced alignment (\Rcode{splicedAlignment=FALSE}). Importantly, we need to set \Rcode{maxHits=50} or similar to also align reads that perfectly match the genome multiple times. This is required because of the miRNAs that are encoded by multiple genes. Reads from such miRNAs would not be aligned and thus their expression would be underestimated if using the default \Rcode{maxHits=1}. An example of such a multiply-encoded miRNA is mmu-miR-669a-5p, which has twelve exact copies in the mm10 genome assembly according to mirBase19.
<<miRNA_qAlign, eval=TRUE>>=
proj3 <- qAlign(sampleFile2, genomeFile, maxHits=50)
alignmentStats(proj3)
@

A more detailed picture of the experiments' quality can be obtained using \Rcode{qQCReport(proj3, "qcreport.pdf")} or similar (see also section~\ref{sec:qQCReport} on page~\pageref{sec:qQCReport}).

\subsubsection{Quantification of small RNA (miRNA) reads}
As with other experiment types, miRNAs are quantified using \Rfunction{qCount}. For this purpose, we first construct a query \Rcode{GRanges} object with the genomic locations of mature miRNAs. The locations can be obtained from the \Biocpkg{mirbase.db} package, or directly from the species-specific gff files provided by the mirBase database (e.g. \url{ftp://mirbase.org/pub/mirbase/19/genomes/mmu.gff3}). For the purpose of this example, the \Biocpkg{QuasR} package provides a small gff file (\Rcode{"mirbaseXX\_qsr.gff3"}) that is formatted as the ones available from mirBase. The gff file contains both the locations of pre-miRNAs (hairpin precursors), as well as mature miRNAs. The two can be discriminated by their \Rcode{"type"}:
...
Please note that the name attribute of the \Rcode{GRanges} object must be set appropriately, so that \Rfunction{qCount} can identify a single mature miRNA sequence that is encoded by multiple loci (see below) by their identical names. In this example, there are no multiply-encoded mature miRNAs, but in a real sample, you can detect them for example with \Rcode{table(names(mirs))}.

The \Robject{preMirs} and \Robject{matureMirs} could now be used as \Rcode{query} in \Rfunction{qCount}. In practise however, miRNA seem to not always be processed with high accuracy. Many miRNA reads that start one or two bases earlier or later can be observed in real data, and also their length may vary for a few bases. This is the case for the synthetic miRNAs used in this example, whose lengthes and start positions have been sampled from a read data set:
<<miRNA_coverage, eval=TRUE, fig=TRUE, width=6, height=4.5>>=
library(Rsamtools)
...
By default, \Rfunction{qCount} will count alignments that have their 5'-end within the query region (see \Rfunarg{selectReadPosition} argument). The 5'-end correspond to the lower (left) coordinate for alignments on the plus strand, and to the higher (right) coordinate for alignments on the minus strand. In order not to miss miRNAs that have a couple of extra or missing bases, we therefore construct a query window around the 5'-end of each mature miRNA, by adding three bases up- and downstream:
...
The resulting extended query is then used to quantify mature miRNAs. Multiple-encoded miRNAs will be represented by multiple ranges in \Robject{matureMirs} and \Robject{matureMirsExtended}, which have identical names. \Rfunction{qCount} will automatically sum all alignments from any of those regions and return a single number per sample and unique miRNA name.
...
\label{sec:Bis}
Sequencing of bisulfite-converted genomic DNA allows detection of methylated cytosines, which in mammalian genomes typically occur in the context of CpG dinucleotides. The treatment of DNA with bisulfite induces deamination of non-methylated cytosines, converting them to uracils. Sequencing and aligning of such bisulfite-converted DNA results in C-to-T mismatches. Both alignment of converted reads, as well as the interpretation of the alignments for calculation of methylation levels require specific approaches and are supported in \Biocpkg{QuasR} by \Rfunction{qAlign} (\Rfunarg{bisulfite} argument, section \ref{sec:qAlign}) and \Rfunction{qMeth} (section \ref{sec:qMeth}), respectively.
...
Finally, \Rfunction{qMeth} allows the retrieval of methylation states for individual molecules (per alignment). This is done by using a \Rfunarg{query} containing a single genomic region (typically small, such as a PCR amplicon) and setting \Rcode{reportLevel="alignment"}. In that case, the return value of \Rfunction{qMeth} will be a list (over samples) of lists (with four elements giving the identities of alignment, C nucleotide, strand and the methylation state). See the documentation of \Rfunction{qMeth} for more details.


\subsection{Allele-specific analysis}
\label{sec:Allele}
All experiment types supported by \Biocpkg{QuasR} (ChIP-seq, RNA-seq and Bis-seq; only alignments to the genome, but not to auxiliaries) can also be analyzed in an allele-specific manner. For this, a file containing genomic location and the two alleles of known sequence polymorphisms has to be provided to the \Rfunarg{snpFile} argument of \Rfunction{qAlign}. The file is in tab-delimited text format without a header and contains four columns with chromosome name, position, reference allele and alternative allele. 
...
The examples below use data provided with the \Biocpkg{QuasR} package, which is first copied to the current working directory, into a subfolder called \Rcode{"extdata"}:
...
\newpage
\section{Description of Individual \Biocpkg{QuasR} Functions}
Please refer to the \Biocpkg{QuasR} reference manual or the function documentation (e.g. using \Rcode{?qAlign}) for a complete description of \Biocpkg{QuasR} functions. The descriptions provided below are meant to give an overview over all functions and summarize the purpose of each one.
...
\label{sec:qAlign}
\Rfunction{qAlign} is the function that generates alignment files in \bam{} format, for all input sequence files listed in \Rfunarg{sampleFile} (see section \ref{sec:SampleFile}), against the reference genome (\Rfunarg{genome} argument), and for reads that do not match to the reference genome, against one or several auxiliary target sequences (\Rfunarg{auxiliaryFile}, see section \ref{sec:AuxFile}).
...
The alignment process produces temporary files, such as decompressed input sequence files or raw alignment files before conversion to \bam{} format, which can be several times the size of the input sequence files. These temporary files are stored in the directory specified by \Rfunarg{cacheDir}, which defaults to the \R{} process temporary directory returned by \Rcode{tempdir()}. The location of \Rcode{tempdir()} can be set using environment variables (see \Rcode{?tempdir}).
...
\item \textbf{Library complexity} shows fractions of unique read(-pair) alignment positions. Please note that this measure is not independent from the total number of reads in a library, and is best compared between libraries of similar sizes. This plot is available for bam input.
\begin{center}
...
\begin{center}
<<qcplotsFig8, eval=TRUE, echo=FALSE, fig=TRUE, height=4, width=8>>=
...
How an alignment overlap with a query region is defined can be controlled by the following arguments of \Rfunction{qCount}:
\begin{itemize}
  ...
  \item \Rfunarg{includeSpliced}: When set to \Rcode{FALSE}, spliced alignments will be excluded from the quantification. This could be useful for example to avoid redundant counting of reads when the spliced alignments are quantified separately using \Rfunarg{reportLevel="junction"}.   
\end{itemize}

\subsubsection{Running modes of \Rfunction{qCount}}
The features to be quantified are specified by the \Rfunarg{query} argument. At the same time, the type of \Rfunarg{query} selects the mode of quantification. \Rfunction{qCount} supports three different types of \Rfunarg{query} arguments and implements three corresponding quantification types, which primarily differ in the way they deal with redundancy, such as query bases that are contained in more than one query region. A fourth quantification mode allows counting of alignments supporting exon-exon juctions:
...
\begin{itemize}
  ...
  \item \Robject{TxDb} query: Used to extract regions from annotation and report alignment counts depending on the value of the \Rfunarg{reportLevel} argument. If \Rcode{reportLevel="exon"}, alignments overlapping each exon in the query are counted. If \Rcode{reportLevel="gene"}, alignment counts for all exons of a gene will be summed, counting each alignment only once even if it overlaps multiple annotated exons of a gene. These are useful to calculate exon or gene expression levels in RNA-seq experiments based on the annotation in a \Robject{TxDb} object. If \Rcode{reportLevel="promoter"}, the \Rfunction{promoters} function from package \Biocpkg{GenomicFeatures} is used with default arguments to extract promoter regions around transcript start sites, e.g. to quantify alignments inf a ChIP-seq experiment.
  \item any of the above or \Rcode{NULL} for \Rcode{reportLevel="junction"}: The \Rfunarg{query} argument is ignored if \Rfunarg{reportLevel} is set to \Rcode{"junction"}, and \Rfunction{qCount} will count the number of alignments supporting each exon-exon junction detected in any of the samples in \Robject{proj}. The arguments \Rfunarg{selectReadPosition}, \Rfunarg{shift}, \Rfunarg{orientation}, \Rfunarg{useRead} and \Rfunarg{mask} will have no effect in this quantification mode.
\end{itemize}
