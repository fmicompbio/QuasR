...
\title{An Introduction to \QuasRlogo{}}
...
\maketitle
% BiocStyle uses package titling, which captures \title{} in \thetitle for use in headers
% since we put an image into \title{}, we have to overwrite \thetitle with plain text
\renewcommand{\thetitle}{An Introduction to \Rpackage{QuasR}}
