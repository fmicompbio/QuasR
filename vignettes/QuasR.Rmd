...
use the current official release version, which will be installed by `biocLite`
if you are using the current release version of **R** (unless you specifically `useDevel()`).
There is also a developmental version of `r Biocpkg("QuasR")` that includes new
features due for the next official release. The development version will be installed
if you are using the development version of **Bioconductor** (see `useDevel()` in
`r Biocpkg("BiocInstaller")`). The official release version always has an even
second number (for example 0.2.1), whereas the developmental version has an odd
second number (for example 0.3.6).

## Loading of `r Biocpkg("QuasR")` and other required libraries
...
has it's own help page, as described in the section "A brief introduction to **R**".
...
section and continue with section "Sample `r Biocpkg("QuasR")` session".
...
from (http://www.r-project.org) as well as `r Biocpkg("QuasR")` (see section "Installation").
...
## Sample `r Biocpkg("QuasR")` session
...
The sequence files to be analyzed are listed in `sampleFile` (see section "Create a sample file" for details).
...
store the resulting alignments (see section "qAlign").
...
of Figure above, see section "File storage locations" for details on storage locations).
...
"Create a sample file" for details), and the reference genome for the alignments
(see section "Select the reference genome").
...
- `preprocessReads` (optional): Remove adapters from start or end of reads, filter
  out reads of low quality, short length or low complexity (section "Sequence data pre-processing").  
- Prepare *samples file*: List sequence files or alignments, provide sample names
  (section "Create a sample file").  
- Prepare *auxiliary file* (optional): List additional reference sequences for
  alignment of reads not matching the reference genome (section "Create an auxiliary file").  
...
- `qQCReport` (optional): Create quality control report with plots on sequence qualities
  and alignment statistics (section "qQCReport").  
- `qExportWig` (optional): Export genomic alignments as wiggle tracks for genome
  browser visualization (section "qExportWig").  
- `qCount`: Quantify alignments in regions of interest (section "qCount").  

Recurrent example tasks that may be part of any typical analysis are described in
section "Example tasks". Example workflows for specific experiment types (ChIP-seq,
RNA-seq and Bis-seq) are described in section "Example workflows".
...
(see section "qAlign" for details). All files generated by `qAlign` are listed here
...
specified to `qAlign` using the `auxiliaryFile` argument (see section "qAlign"
for details). The format of the auxiliary file is similar to the one of the sample
file described in section "Create a sample file": It contains two columns with
...
documentation (see `?preprocessReads`) or in the section "preprocessReads".
...
in section "Sequence data pre-processing". Also, a sample file (section "Create a sample file")
that lists all sequence files to be analyzed has been prepared. A `fasta` file
with the reference genome sequence(s) is also available (section "Select the reference genome"),
as well as a auxiliary file for alignment of reads that failed to match the reference genome (section "Create an auxiliary file").
...
(see section "qAlign"). We start by aligning the reads using `qAlign`:
...
Currently available plots are described in section "qQCReport" and following.
...
or several `bam` file names (for details see section "alignmentStats").
...
of a `fasta` file as reference genome (see section "Select the reference genome").
...
"preprocessReads" for more details), which for each input sequence file will
...
(section "RNA-seq: Gene expression profiling"), alignment time can be reduced by
...
`qQCReport(proj3, "qcreport.pdf")` or similar (see also section "qQCReport").
...
`r Biocpkg("QuasR")` by `qAlign` (`bisulfite` argument, section "qAlign") and 
`qMeth` (section "qMeth"), respectively.
...
Below is an example of a SNP file, also available from `system.file(package="QuasR", "extdata", "hg19sub\_snp.txt")`:
...
(section "ChIP-seq: Measuring protein-DNA binding and chromatin modifications"),
but this time using a `snpFile`:
...
\newpage
# Description of Individual `r Biocpkg("QuasR")` Functions
...
input sequence files listed in `sampleFile` (see section "Create a sample file"),
...
(`auxiliaryFile`, see section "Create an auxiliary file").

The reference genome can be provided either as a `fasta` sequence file or as a
`BSgenome` package name (see section "Select the reference genome"). If a `BSgenome`
...
"qProject class" for methods to access the information contained in a `qProject`
object).
...
ChIP-seq example in section "ChIP-seq: Measuring protein-DNA binding and chromatin
modifications" (except for the fragment size distributions which are based on an
...
quantification mode allows counting of alignments supporting exon-exon juctions:  
  ...
  - `GRanges` query: Overlapping alignments are counted separately for each coordinate
    ...
    quantify ChIP-seq alignments in promoter regions (see section "ChIP-seq: 
    Measuring protein-DNA binding and chromatin modifications"), or gene expression
    levels in an RNA-seq experiment (using a 'query' with exon regions named by gene).  
...
in section "qCount" and from the `qProfile` function documentation.
...
"Allele-specific analysis"), cytosines (or CpGs) that overlap a sequence 
