...
In the following example, we truncate the reads by removing the three bases from the 3'-end (the right side), remove the adapter sequence \texttt{AAAAAAAAAA} from the 5'-end (the left side) and filter out reads that are shorter than 14 bases (after truncation and adapter removal) or contain more than 2 \texttt{N} bases:
...
Currently available plots are described in section \ref{sec:qQCReport} on page \pageref{sec:qQCReport}.
