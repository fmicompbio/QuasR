...
\date{Modified: 7 January, 2012. Compiled: \today}
...
\hline
AuxName & FileName                       \\
phiX174 & phage\_genomes/NC\_001422.1.fa \\
\hline
\end{tabular}
\end{center}
\vskip 1em
...
In order to create the \Robject{qProject} object, a sample file (section \ref{sec:SampleFile}) and a reference genome (section \ref{sec:refGenome}) will be needed. Optionally, you can also include auxiliary references (section \ref{sec:AuxFile}).
