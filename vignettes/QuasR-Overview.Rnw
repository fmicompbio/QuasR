...
\usepackage{url}
\usepackage[breaklinks]{hyperref} % remove to suppress links
...
\date{Modified: May 22 2013. Compiled: \today}
...
\QuasR{} works with data (sequences and alignments, reference genome, etc.) that are stored as files on your storage (the gray cylinder on the lower left of Figure \ref{fig:QuasR-scheme}, see section \ref{sec:FileStorage} for details on storage locations). \QuasR{} does not need a database management system, or these files to be named and organized according to a specific scheme.
