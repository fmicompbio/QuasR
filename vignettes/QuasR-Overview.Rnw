...
\date{Modified: December 21, 2012. Compiled: \today}
...
\item \textbf{Library complexity} shows fractions of unique read(-pair) alignment positions. This plot is available for bam input.
\begin{center}
